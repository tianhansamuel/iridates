...
for i1 in range(0,LY+1):
  py=4*pi*i1/(3*LY)-2*pi/3      
  ymin=-4*pi/(3*sqrt(3))+1.0/(sqrt(3))*fabs(py)
  ymax=4*pi/(3*sqrt(3))-1.0/(sqrt(3))*fabs(py)

  for i2 in range(0,LX+1):
   px=(ymax-ymin)*i2*1.0/LX+ymin

   for i3 in range(0,LY+1):
     qy=4*pi*i3/(3*LY)-2*pi/3      
     y1min=-4*pi/(3*sqrt(3))+1.0/(sqrt(3))*fabs(qy)
     y1max=4*pi/(3*sqrt(3))-1.0/(sqrt(3))*fabs(qy)
     for i4 in range(0,LX+1):
       qx=(y1max-y1min)*i4*1.0/LX+y1min
	...
	REP1=REP.dot(P2)
        RR=P3.dot(HH)
        RRR=RR.dot(P4)
        REP1=REP1*(cos(kx*rx+ky*ry)+1j*sin(kx*rx+ky*ry))+RRR*(cos(kx*rx+ky*ry)-1j*sin(kx*rx+ky*ry))
